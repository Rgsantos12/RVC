...
def initParams():
    ...
    # Voice arguments
    parser.add_argument("--path_to_index", type=str, help="index voice path",
                        default="logs/AUTO (WALL-E_Latin American Dub) - Weights.gg Model/auto_model.index",
                        required=False)
    parser.add_argument('--is_male_voice', type=str2bool, nargs='?', const=True, default=True,
                        required=False, help="whether the downloaded voice is male or not (low pitched/high pitched)")
    # Audio configurations
    parser.add_argument("-i", "--inp_dir", type=str, help="input directory path",
                        default="processed_audios_by_uvr5/", required=False)

    # You can add more choices for input format
    parser.add_argument("--inp_format", type=str, help="input audio format",
                        default="wav", required=False)

    parser.add_argument("-o", "--out_dir", type=str, help="output directory path",
                        default="output_audios/AUTO", required=False)
...
if __name__ == "__main__":
    ...
    for audio_file in glob.glob(os.path.join(args.inp_dir, f"*.{args.inp_format}")):
        ...
        audio_new_name = f'fake_{audio_name.split("-")[0]}_rvc_' + os.path.basename(use_voice).split(".")[0] + \
                         "_audio.wav"
